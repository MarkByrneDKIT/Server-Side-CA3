...
function username_validation() {
    'use strict';
    var cpu_name = document.getElementById("username");
    var cpu_value = document.getElementById("username").value;
    var cpu_length = cpu_value.length;
    if (cpu_length < 4 || !cpu_length > 20) {
        document.getElementById('username_err').innerHTML = 'Username must be 4 chracters long minimum and 20 characters maximum.';
        cpu_name.focus();
...
function password_validation() {
    'use strict';
    var idformat = /^(?=\D*\d)\S{6,}$/;
    var id_name = document.getElementById("password");
    var id_value = document.getElementById("password").value;
    var id_length = id_value.length;
    if (!id_value.match(idformat) || id_length === 0) {
        document.getElementById('password_err').innerHTML = 'The Password must be longer than 6 letters and have a number.';
        id_name.focus();
